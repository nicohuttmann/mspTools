...
#'
#' @examples
UniProt_to_T2GO <- function(data_UniProt_GO, 
                            term_column = "Gene Ontology IDs", 
                            ontologies = c("CC", "BP", "MF"), 
                            GO_col = "ids", 
                            sep = "; ") {
  
  if (!hasArg(data_UniProt_GO)) stop("No UniProt GO data given.")
  
  list_T2GO <- purrr::map(
    ontologies %>% 
      setNames(., .), 
    \(x) data_UniProt_GO %>% 
      dplyr::mutate(!!term_column := 
                      strsplit(!!rlang::sym(term_column), sep) %>% 
                      GO_filter_ontology(ontology = x) %>% 
                      GO_add_ancestors() %>% 
                      GO_append_description() %>% 
                      purrr::map_chr(\(x) paste(x, collapse = sep))) %>% 
      extract_UniProt_GO2TERM2GENE(GO_col = GO_col))
